...
public:
	struct Slot
	{
		...
		bool RemoveWatcher(BfpFileWatcher* watcher)
		{
			if (watcher->mFlags & BfpFileWatcherFlag_IncludeSubdirectories)
				mHandleSubdirs--;
		...
		void SendEvents(InotifyWorker* manager, const inotify_event* event, BfpFileChangeKind kind, const inotify_event* pairedEvent = NULL, Slot* pairedEventSlot = NULL)
		{
...
public:
	...
	{
		return InotifyWatchPath(path, NULL, -1);
	}
...
private:
    ...
    void HandleDirAdd(const inotify_event* event, Slot* slot, const StringView& path, bool moved)
    {
    	...
    	Slot* addedSlot;
        auto watchHandle = InotifyWatchPath(fullPath, &addedSlot, slot->mWd);
    ...
    WatchHandle InotifyWatchPath(const String& path, Slot** resultSlot, int parentWd);

	void DeleteFileWatcher(WatchHandle handle, BfpFileWatcher* fileWatch);
...
String InotifyWorker::GetRelativePath(const Slot *slot, WatchHandle parent, const StringView &path)
{
	Slot* parentSlot;
...
String InotifyWorker::GetRelativePath(const Slot *slot, int parent, const StringView &path)
{
...
void InotifyWorker::DispatchEvent(const inotify_event *event, Array<const inotify_event *> unhandledEvents)
{
	// Watch removed
...
void InotifyWorker::DispatchUnhandledEvent(const inotify_event *event)
{
	Slot* slot;
	if (!mWatchSlots.TryGetValue(event->wd, &slot))
		return;
...
WatchHandle InotifyWorker::InotifyWatchPath(const String &path, Slot** resultSlot, int parentWd)
{
...
void InotifyWorker::DeleteFileWatcher(WatchHandle handle, BfpFileWatcher *fileWatch)
{
	defer ( delete fileWatch );

	Slot* slot;
	if (mWatchSlots.TryGetValue(handle.mIdx, &slot))
	{
		...
		if ((slot->mWatchers.IsEmpty()))
		{
			...
			mWatchSlots.Remove(handle.mIdx);
		}
	}
...
void InotifyWorker::WatchSubdirectories(const String& path, Slot* slot, bool sendEvents, int parentWd)
{
	...
	while (workList.size() > 0)
	{
		...
		while ((dp = readdir(current)) != NULL)
		{
			...
			Slot* resultSlot;
			WatchHandle watchHandle = InotifyWatchPath(currentPath, &resultSlot, parentWd);
			if (watchHandle.IsValid())
			{
...
static void* WorkerProcThunk(void* _this)
{
	...
	((InotifyWorker*)_this)->WorkerProc();
	return NULL;
}

bool InotifyWorker::Init()
{
...
BfpFileWatcher * InotifyFileWatchManager::WatchDirectory(const char *path, BfpDirectoryChangeFunc callback, BfpFileWatcherFlags flags, void *userData, BfpFileResult *outResult)
{
	...
	WatchHandle watchHandle = mWorker.AddWatchPath(path);
	if (!watchHandle.IsValid())
	{
	...
	mWorker.AddWatcher(fileWatcher);
